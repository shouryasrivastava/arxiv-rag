...
\begin{itemize}
  ...
  \item \textit{How is it done today, and what are the limits of current
        practice?} General LLMs hallucinate citations; search engines return
        links but cannot synthesise answers; existing scholarly tools
        (Semantic Scholar, Elicit) require cloud APIs.
...
dense retriever (DPR~\citep{dpr2020}) with a seq2seq generator.
\citet{realm2020} extended retrieval to the pre-training stage, while
\citet{atlas2022} showed that a smaller (11B) RAG model can match GPT-3 (175B)
on knowledge benchmarks when retrieval quality is high.
...
vectors in a shared embedding space and retrieve by maximum inner product search.
...
Ollama~\citep{ollama2024} wraps these models in a unified REST API with
4-bit quantisation (Q4\_K\_M), reducing the memory footprint of Llama~3.2~3B
to approximately 2\,GB and making CPU-only deployment viable.
...
We index the 50{,}000 most recent papers in eight CS/ML categories
...
Each record contributes its title and abstract (average 175 words,
well within the 512-token encoder limit) as a single indexable document.
...
\begin{tikzpicture}[
  box/.style  = {rectangle, rounded corners=4pt, draw=black!50, fill=#1,
                 text centered, minimum height=0.85cm, minimum width=2.7cm, font=\small},
...
\draw[decorate, decoration={brace,amplitude=5pt}, thick, color=blue!50]
  ...
  node[midway,xshift=-0.9cm,rotate=90,anchor=south,font=\small\bfseries,color=blue!65]{Ingestion};
...
\draw[decorate, decoration={brace,amplitude=5pt,mirror}, thick, color=purple!50]
  ...
  node[midway,xshift=0.9cm,rotate=-90,anchor=south,font=\small\bfseries,color=purple!65]{Query};
\end{tikzpicture}
\caption{ArXiv Research Assistant architecture.
  The ingestion pipeline (left) indexes papers once; the query pipeline (right)
  embeds each user question, retrieves the top-$k$ papers, and streams a
...
Papers are downloaded in a streaming fashion from the HuggingFace
\texttt{Cornell-University/arxiv} dataset (with automatic fallback to the
official ArXiv export API).
Each batch of 512 papers is embedded with \textit{all-MiniLM-L6-v2}
(384-d, cosine-normalised) and immediately upserted into ChromaDB~\citep{chromadb2023},
so the index is queryable from the very first batch rather than after full
ingestion completes.
The pipeline supports \textbf{incremental resume}: on restart it fetches
existing document IDs and skips already-indexed papers.
...
175 words, $\sigma\approx 45$), matching the 512-token context of the
embedding model.
No additional chunking is required at the abstract level.
Author strings are truncated to 500 characters to avoid ChromaDB metadata
size limits.

\subsection{Query Pipeline}

\begin{enumerate}
  \item \textbf{Query embedding.} The user question is encoded by the same
        \textit{all-MiniLM-L6-v2} encoder (unit-norm, 384-d, $<$25\,ms).
  \item \textbf{Dense retrieval.} ChromaDB returns the top-$k$ papers by
        ...
        via a ChromaDB \texttt{\$contains} metadata predicate.
  \item \textbf{Prompt construction.} A structured prompt is built containing:
        (a) a strict system instruction, (b) numbered abstract blocks with
        title/authors/date/URL, and (c) the user question.
  \item \textbf{Streaming generation.} The prompt is sent to Ollama's REST
        endpoint (\texttt{/api/generate}, \texttt{stream: true}).
        Tokens are yielded one-by-one and displayed live in Streamlit.
  \item \textbf{Citation cards.} Each retrieved paper is displayed as a card
        with title, relevance score ($1-d_\text{cosine}$), categories, date,
        and a clickable \texttt{arxiv.org/abs/<id>} link.
\end{enumerate}
...
\end{quote}
Generation temperature is set to 0.1 (top-$p$ = 0.9) to minimise creative
deviation.
Across our 30-query evaluation, the system produced \textbf{zero hallucinated
citations}; every bracketed reference corresponded to a real, retrievable paper.
...
Dense retrieval outperforms BM25 by +24\,pp at P@1, confirming that semantic
similarity better captures research intent than keyword overlap.
Retrieval latency is under 30\,ms; end-to-end latency on a CPU-only GCP
\texttt{n2-standard-4} instance is $\approx$51\,s (embedding: 22\,ms,
retrieval: 8\,ms, LLM generation: $\approx$51\,s for 200 tokens).
...
A startup script (a) waits for Ollama readiness, (b) pulls the model, (c)
runs a single-token warmup request to load weights into RAM, and (d) launches
ingestion in the background so Streamlit is immediately available.
The system is deployed on a GCP \texttt{n2-standard-4} VM at
\texttt{http://34.26.47.93:8501}.
...
\begin{itemize}
  \item \textbf{Repository (public):}
        ...
        \texttt{docker-compose.yml}, \texttt{requirements.txt}, and
        this write-up under \texttt{writeup/}.
  \item \textbf{Live demo endpoint:}
        \texttt{http://34.26.47.93:8501} — publicly accessible Streamlit UI
        backed by a GCP \texttt{n2-standard-4} VM running the full Docker stack.
